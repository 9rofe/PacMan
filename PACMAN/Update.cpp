...
void Engine::Update()
{
	...
	for (unsigned int counter = 0; counter < m_wallsMap.size(); counter++)
	{
		...
		{
			Rect<float> enemyRect(m_Enemies.at(counter).GetLocation(), {80.0, 80.0});
			bool collidesEnemy = enemyRect.intersects(playerRect);
			if (collidesEnemy)// && m_Enemies.at(counter).getDirection() == Direction::UP)
			{
				m_Player->SetLives(-1);
				m_Player->SetLocation(Vector2f(resolution.x / 2.0f, resolution.y / 1.1f));
	...
	while (dotsIter != m_dotsMap.end()) {
		...
		else if(m_dotsMap.size() == 0 && m_fruitMap.size() == 0)
		{
			//reset
		}
		else
